  ...
  interface vind_panelgeo
    ...
    contains
      ...
      subroutine rot_wing(wing_array,pts,origin,order)  
        ...
        real(dp), intent(in), dimension(3) :: pts  ! pts => phi,theta,psi
      ...
      function vind_onwake_bywake(bywake_array,wake_array,fil,fil_gam) result(vind_array)
        type(wakepanel_class), intent(in), dimension(:,:) :: bywake_array
        type(wakepanel_class), intent(in), dimension(:,:) :: wake_array
        real(dp), dimension(3,size(wake_array,1),size(wake_array,2)+1) :: vind_array
        type(vf_class), intent(in) :: fil
        real(dp), intent(in) :: fil_gam
        integer :: i,j,rows,cols
        ...
        do j=1,cols
          do i=1,rows
            ...
            vind_array(:,i,j)=vind_array(:,i,j)+fil%vind(wake_array(i,j)%vr%vf(2)%fc(:,1))*fil_gam
          enddo
        enddo
        !$omp end parallel do
        ...
        do i=1,rows
          ...
          vind_array(:,i,cols+1)=vind_array(:,i,cols+1)+fil%vind(wake_array(i,cols)%vr%vf(3)%fc(:,1))*fil_gam
        enddo
        !$omp end parallel do
